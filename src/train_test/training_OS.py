...
def train_dl_model_OS(epoch, dataset, model, train_loader, loss_fn, optimizer, model_dir, model_name,
    ...
    with tqdm(train_loader, unit="batch") as tepoch:
        #with autograd.detect_anomaly():
                    ...
                    for batch_idx, (X, Z, W, Y, X_mask, Y_mask) in enumerate(tepoch):
                        ...
                        Y_hat, aux_loss = model(X, Z, W, X_mask, mc_dropout = True,
                                      get_aux_loss = True)
                    ...
                    with torch.no_grad():
                        ...
                        if epoch == 0 and plot_arch is True:
                            print("Plotting model's architecture...")
    ...
    with tqdm(train_loader, unit="batch") as tepoch:
        #with autograd.detect_anomaly():
                    ...
                    for batch_idx, (X, Z, W, Y, X_mask, Y_mask) in enumerate(tepoch):
                        ...
                        [Y_hat, aux_loss], K_lat_lon = model(X, Z, W, X_mask, teacher_forcing = teacher_forcing,
                                                  mc_dropout = True, get_aux_loss = True, K_out = True)
                        ...
                        if physics_guide < physics_guide_alpha[epoch]:
                            ...
                            physics_loss = loss_physics_fn(Y_hat,
                                                            ...
                                                            dataset = dataset)
                            print(f"Training_data_loss: {data_loss.item()} (aux_moe: {aux_loss}) --- Training_physics_loss: {physics_loss.item()}")
                            ...
                            print(f"Training_data_loss: {data_loss.item()} (aux_moe: {aux_loss})")
                            loss = data_loss
