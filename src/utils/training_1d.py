...
def train_dl_pde_model_1d(epoch, ds, model, train_loader, optimizer,
    ...
    with tqdm(train_loader, unit="batch") as tepoch:
                    for batch_idx, (x, z, w_values, y, x_mask, y_mask) in enumerate(tepoch):
                        ...
                        ## QUA
                        ## Da gestire cpoint per batch e per istanza
                        
                        x_cpoints = torch.repeat_interleave(x_cpoints, num_cpoint_instance, dim=0).to(device) # repeat instances 
                        x_mask_cpoints = torch.repeat_interleave(x_mask_cpoints, num_cpoint_instance, dim=0).to(device)
                        w_cpoints = [torch.repeat_interleave(w_cpoints[0], num_cpoint_instance, dim=0).to(device),
                        ...
                        z_cpoints = torch.tensor(z_cpoints, requires_grad=True).to(torch.float32).to(device)
                        
                        print("bef mva", z_cpoints.moveaxis(-1, 2).shape)
                        ...
                        y_hat_pde, = y_hat_pde.reshape(y_hat.shape[0],9,num_cpoint_instance, y_hat.shape[-1])
                        hydro_cond_hat = hydro_cond_hat.reshape(y_hat.shape[0],9,num_cpoint_instance, y_hat.shape[-1], 2)
                        
                        loss_pde = disc_physics_loss(y_hat_pde[:,0,:,:], y_hat_right = y_hat_pde[:,1,:,:], y_hat_left = y_hat_pde[:,2,:,:],
                                                    y_hat_up = y_hat_pde[:,3,:,:], y_hat_down = y_hat_pde[:,4,:,:],
                                                    y_hat_two_right = y_hat_pde[:,5,:,:], y_hat_two_left = y_hat_pde[:,6,:,:],
                                                    y_hat_two_up = y_hat_pde[:,7,:,:], y_hat_two_down = y_hat_pde[:,8,:,:],
                                                    k_lat = hydro_cond_hat[:,0,:,:,0], k_lat_up = hydro_cond_hat[:,3,:,:,0], k_lat_down = hydro_cond_hat[:,4,:,:,0],
                                                    k_lon = hydro_cond_hat[:,0,:,:,1], k_lon_right = hydro_cond_hat[:,1,:,:,1], k_lon_left = hydro_cond_hat[:,2,:,:,0],
                                                    step = fdif_step,
                                                    g =  g,
                                                    S_y = S_y)
