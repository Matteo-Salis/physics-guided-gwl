    ...
    with torch.no_grad():
                with tqdm(test_loader, unit="batch") as tepoch:
                            ...
                            for batch_idx, (x, z, w_values, y, x_mask, y_mask) in enumerate(tepoch):
                                ...
                                z = z.to(device)
                                weather_coords_batch = weather_coords.expand(w_values.shape[0], -1, -1, -1)
...
def test_dl_pde_model_1d(epoch, ds, model, test_loader, optimizer,
                          num_cpoint_batch,
                          num_cpoint_instance,
                ...
                with tqdm(test_loader, unit="batch") as tepoch:
                    ...
                    for batch_idx, (x, z, w_values, y, x_mask, y_mask) in enumerate(tepoch):
                        ...
                        print("Test_data_loss: ", loss_data.item(), end = " --- ")
                        ...
                        y_hat_pde, = y_hat_pde.reshape(num_cpoint_batch, num_cpoint_instance, 9, y_hat.shape[-1])
                        hydro_cond_hat = hydro_cond_hat.reshape(num_cpoint_batch, num_cpoint_instance, 9, y_hat.shape[-1], 2) #(num_cpoint_batch, num_cpoint_instance, 9, 3)
                        ...
                        loss_pde = disc_physics_loss(y_hat = y_hat_pde[:,:,0,:],
                                                    ...
                                                    k_lon_right = hydro_cond_hat[:,:,1,:,1], k_lon_left = hydro_cond_hat[:,:,2,:,1],
                                                    k_lat_up = hydro_cond_hat[:,:,3,:,0], k_lat_down = hydro_cond_hat[:,:,4,:,0],
                                                    step = fdif_step,
                                                    g = g,
                                                    S_y = S_y)
